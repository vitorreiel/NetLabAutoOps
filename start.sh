...
if [[ "$confirmacao" =~ ^(1|01)$ ]]; then
  ...
  sleep 1
  echo -e "\n\033[1;34m- [ 1 ] : Topologia de Rede Linear \033[0m"
  sleep 0.5
  echo -e "\n\033[1;34m- [ 2 ] : Topologia de Rede Single \033[0m"
  sleep 0.5
...
EOF
  ...
  1|01)
    while true; do
      echo "Quantos switches você deseja criar?"
      read num_switches

      echo "Quantos hosts serão conectados a esses switches?"
      read num_hosts
      ...
      if [ "$num_switches" -eq "$num_hosts" ]; then
          ...
          echo "A quantidade de switches e hosts deve ser a mesma. Por favor, tente novamente."
      fi
    done
  esac
  ...
  case $topologia in
    2|02)
      echo "Quantos hosts serão conectados ao único switch?"
      read num_hosts
      num_switches=1
  esac
  ...
  case $topologia in
    3|03)
    echo "Quantos switches você deseja no nível 1?"
    read num_switches_lvl1
    echo "Quantos switches você deseja no nível 2?"
    read num_switches_lvl2
    echo "Quantos hosts serão conectados a cada switch do nível 2?"
